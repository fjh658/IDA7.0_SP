...
**
** $QT_END_LICENSE$
** https://bugreports.qt.io/browse/QTBUG-52591
****************************************************************************/
...
#ifdef DEBUG_KEY_BINDINGS_MODIFIERS
  qDebug("Qt: internal: **Mapping modifiers: %d (0x%04x)", (int)keys,
         (int)keys);
#endif
  int ret = 0;
  for (int i = 0; qt_mac_modifier_symbols[i].qt_code; i++) {
...
static int qt_mac_get_key(int modif, const QChar &key, int virtualKey) {
  ...
  qDebug("**Mapping key: %d (0x%04x) - %d (0x%04x)", key.unicode(),
  ...
  for (int i = 0; qt_mac_keyboard_symbols[i].qt_code; i++) {
    if (qt_mac_keyboard_symbols[i].mac_code == key) {
       ...
       */
      if (qt_mac_keyboard_symbols[i].qt_code == Qt::Key_Tab &&
  ...
  // oh well
#ifdef DEBUG_KEY_BINDINGS
  qDebug("Unknown case.. %s:%d %d[%d] %d", __FILE__, __LINE__, key.unicode(),
...
bool QCocoaKeyMapper::updateKeyboard() {
  ...
  uchrData =
      data ? reinterpret_cast<const UCKeyboardLayout *>(CFDataGetBytePtr(data))
           : 0;
...
void QCocoaKeyMapper::updateKeyMap(unsigned short macVirtualKey,
                                   QChar unicodeKey) {
  updateKeyboard();
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    const UInt32 keyModifier =
        ((qt_mac_get_mac_modifiers(ModsTbl[i]) >> 8) & 0xFF);
    OSStatus err =
        UCKeyTranslate(keyboard_layout_format.unicode, macVirtualKey,
                       kUCKeyActionDown, keyModifier, keyboard_kind, 0,
                       &keyboard_dead, buffer_size, &out_buffer_size, buffer);
  ...
  for (int i = 0; i < 16; ++i) {
    qDebug("    [%d] (%d,0x%02x,'%c')", i, keyLayout[macVirtualKey]->qtKey[i],
...
QList<int> QCocoaKeyMapper::possibleKeys(const QKeyEvent *event) const {
  QList<int> ret;
  const_cast<QCocoaKeyMapper *>(this)->updateKeyMap(event->nativeVirtualKey(),
                                                    QChar(event->key()));

  KeyboardLayoutItem *kbItem = keyLayout[event->nativeVirtualKey()];

  if (!kbItem) // Key is not in any keyboard layout (e.g. eisu-key on Japanese
               // keyboard)
    return ret;
